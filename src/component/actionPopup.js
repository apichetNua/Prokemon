...
import Conteniner from "./Conteiner";
...
let dataArray = [];
let updatePokemon;
const Actionpopup = ({ dispatch, counters }) => {
  ...
  function AddPost(id, name, hp, str, weak, dmg, img,happiness, KeyId) {
    ...
    if (index >= 1) {
      counters.counters.map((counter) => (
        <Conteniner key={counter.id} counter={counter} />
      ));
  ...
  function togglePopup() {
    ...
    image.forEach((value)=> value.map((data,index)=>console.log(index,data)))
    ...
    <div>
      <div className="popup" id="popup-1">
        ...
        <div className="content">
          <img className="searchImage" src="./search.png" />
      ...
      <button className="btnPush" onClick={() => togglePopup()}><h1>+</h1></button>
    </div>
  );
};
